  ...
  try {
    ...
    if (userExists) {
      ...
      const { count, rows } = await Property.findAndCountAll({
        where: { creator_id: id },
        attributes: ["id", "title", "description", "photo", "propertyType"],
        order: [
      ...
      rows.forEach((property) => {
        ...
        if (bridgeObject.hasOwnProperty(property.propertyType)) {
          bridgeObject[property.propertyType] += 1;
        } else {
          bridgeObject[property.propertyType] = 1;
        }
      ...
      // convert brigdeObject to an array of objects
      //{type: count, type: count} --> [[type: count], [type: count]] --> [{type: count}, {type: count}]
      const result = Object.entries(bridgeObject).map(([type, count]) => ({
        [type]: count,
      }));

      //sending de bridgeObject to the allMetrics.propertyType.propertiesList array.
      allMetrics.propertyType.propertiesList = result;
